module opg1
    ...
    // t -> NODE(5, NODE(3, LEAF, NODE(2, LEAF, LEAF)), NODE(1,NODE(4, LEAF, LEAF),LEAF))
    ...
    that takes a function f of three arguments, a value b, and a tree t, 
    and that recurses over the tree by returning b when it meets a LEAF and that applies f when it meets a NODE.*)
    let rec treefold f b  = function
        | LEAF -> b 
        | NODE(x,y,z) -> f x (treefold f b y) (treefold f b z) 
